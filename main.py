...
def classificar(relacao, A):
    ...
    symmetric = all((y, x) in relacao for x, y in relacao)
    transitive = all((x, w) in relacao for x, y in relacao for z, w in relacao if y == z)
    reflexive = all((a, a) in relacao for a in A)
    irreflexive = all((a, a) not in relacao for a in A)
